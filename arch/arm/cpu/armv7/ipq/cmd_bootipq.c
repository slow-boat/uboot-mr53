...
#ifdef CONFIG_IPQ_APPSBL_DLOAD
		ulong panic_nand_off  =  0x07f80000;
		ulong panic_nand_size =  IPQ_PANIC_DUMP_SIZE;
		ulong panic_mem_addr = IPQ_PANIC_DUMP_ADDR;

		sprintf(cmd, "nand erase 0x%x 0x%x", panic_nand_off, panic_nand_size);
		run_command(cmd, 0);

		sprintf(cmd, "nand write 0x%x 0x%x 0x%x", panic_mem_addr, panic_nand_off, panic_nand_size);
		run_command(cmd, 0);

		printf("PANIC ADDR 0x%x SIZE 0x%x\n", panic_mem_addr, panic_nand_size);
		printf("\n");
#endif
		run_command("reset", 0);
	}

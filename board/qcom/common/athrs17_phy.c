...
{
	uint32_t reg_val; 
	printf("s17_rd_phy: add: %x reg: %x\n", phy_addr, reg_addr); 
	reg_val = (uint32_t) phy_reg_read(0, phy_addr, reg_addr);
	return (reg_val);
}

void s17_wr_phy(unsigned int phy_addr, unsigned int reg_addr, unsigned int write_data)
{
	printf("s17_wr_phy: add: %x reg: %x\n", phy_addr, reg_addr); 
...
void athrs17_reg_init(ipq_gmac_board_cfg_t *gmac_cfg)
{
	...
	data = athrs17_reg_read(S17_MASK_CTRL_REG);
	if ( ((data & 0xff00) != 0x1200) || ((data & 0xff00) != 0x1300) )
	{
		printf("Not AR832x or QCA833x, skipping init functions...%/8x \n", data);
		return;
	}
